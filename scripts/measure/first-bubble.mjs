 ...
 * 另用 CDP 的 Network 事件记录同一请求"发出 → 首个数据块（首个 SSE 帧）到达"的网络时刻，
 * 用来区分"上游首字延迟"和"前端等第一行写完的时间"（⚠️ 上游开启思考模式时，答案会在思考结束后整段一次性到达，
 * 首个气泡与全文完成几乎同时，见 docs/interview.md#thinking-mode）。
...
async function main() {
  ...
  const net = { requestId: null, sent: 0, firstData: 0 };
  cdp.on('Network.requestWillBeSent', (event) => {
    if (event.request.method === 'POST' && event.request.url.endsWith('/chat')) {
      Object.assign(net, { requestId: event.requestId, sent: event.timestamp, firstData: 0 });
    }
  });
  ...
  try {
    ...
    for (let i = 0; i < runs; i += 1) {
      ...
      results.push({ first: t1 - t0, done: t2 - t0, firstByte });
      console.log(
        `run ${i + 1}: 首个气泡 ${(t1 - t0).toFixed(0)} ms，全文完成 ${(t2 - t0).toFixed(0)} ms；` +
          `网络：首个数据块 ${firstByte.toFixed(0)} ms`,
      );
    }
  } finally {
    await browser.close();
