...
| 密码哈希                   | ① 直接用 `bcrypt` ② `passlib[bcrypt]`                                                                                      | ② passlib 1.7.4 自 2020 年停更，与 bcrypt ≥ 4.1 不兼容（读不到 `bcrypt.__about__`，登录时 `AttributeError`）；多一层没有收益                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | ① `security.py` 的 `hashpw` / `checkpw` 各一行；密码规则 `^[\x21-\x7E]{6,64}$`（可打印 ASCII）天然不超过 bcrypt 72 字节上限，前后端各写一遍同一条正则                           |
...
- **结果与代价**：`http.test.ts`、`RequireAuth.test.tsx`、`LoginPage.test.tsx`、后端 `test_me_requires_valid_token`、`test_stop_other_users_conversation_404` 等覆盖；E2E `auth.spec.ts` 走"未登录跳转 → 演示账号登录 → 退出"。代价：退出登录时不会主动 abort 正在进行的流（流继续写 store，结束后的重拉因无 token 得到 401，`toastError` 对 401 静默）；`StreamingState.controller` 已保留，需要时一行即可接上。

### context-window

**上下文截断（最近 40 条）与每日额度**（`backend/app/routers/chat.py` 第 24–52、186–196 行；`models.py` `ContextEntry`）

- **背景与问题**：角色提示词 4–6 千字、固定系统提示词 1,122 字、默认世界观 256 字，以陈千语为例每次请求的固定 prompt 约 7,374 字（粗估 4.9k token）；对话越长 `prompt_tokens` 越贵。演示账号公开在登录页，必须限制每人每天的调用量。同时 spec 要求"清空消息"与"清空上下文"互不影响：清了界面 AI 仍记得，清了上下文界面仍在。
...
- **结果与代价**：两条用例通过；`scripts/measure/prompt-tokens.py` 已能连续发 60 条并读 `usage` 帧画曲线（mock 无 usage 帧，真实数据待接 Key）。代价：按条数截断不等于按 token 截断，一条超长回复仍可能撑大 prompt；额度按"条"而不是按 token，也不区分 mock 与真实调用。
...
四项已有实测数据，两项脚本已备、数据待接真实 DeepSeek Key 后补。所有数据来自 `docs/notes/measurements.md`、`characters.md`、`chat.md`、`integration.md`。
...
**首个 AI 气泡出现时间（流式按行 vs 等全文）：脚本已备，真实数据待补**

- **测量方法**：`scripts/measure/first-bubble.mjs`（Playwright，从 `e2e` 工作区 `createRequire` 解析 `@playwright/test`）。接口登录写 `localStorage['baker.token']` → 为角色新建空会话 → 打开 `/` 选中该会话 → 页面内装探针：输入框 `keydown` 捕获阶段记 t0，`MutationObserver` 记第一个 `svg:not([role="status"]) > rect.fill-bubble-other` 出现为 t1（排除同样带 `fill-bubble-other` 的加载气泡）、`[role="status"][aria-label="正在回复"]` 出现后消失为 t2（前端处理完 `[DONE]`，即"等全文再显示"方案下用户能看到第一句话的时刻）→ `fill` + `Enter` → 重复 `--runs` 次取中位数 → 删除会话。三个时刻都在页面内同一时钟取，Node 只负责等 `t2 > 0` 再读回，不含 Playwright 轮询间隔。
...
- **真实 DeepSeek 下待补**：t1 主要由上游首 token 延迟 + 第一行长度决定，t2 由全文长度决定，两者差值才是"流式按行"相对原项目"等全文再逐条播放"的收益。运行前置：后端去掉 `AI_MOCK`、设 `DEEPSEEK_API_KEY`，`CORS_ORIGINS` 含前端端口；前端以对应 `VITE_API_BASE_URL` 构建后 `vite preview`。

  ```bash
  cd backend && JWT_SECRET=<≥32字节> CORS_ORIGINS=http://localhost:5182 DATABASE_URL=sqlite:////tmp/measure.db \
    .venv/bin/uvicorn app.main:app --port 8022
...
**单次请求 prompt_tokens 曲线（上下文截断前后）：脚本已备，真实数据待补**

- **测量方法**：`scripts/measure/prompt-tokens.py`（仅标准库 `urllib`，任何 python3 直接运行）。登录 → 新建会话 → 连续发送 N 条"第 i 条：请用一句话回复我。" → 逐行读 SSE 取 `usage` 帧的 `prompt_tokens` → 打印第 1、10、20、40、50、60 条的值与完整 CSV → `finally` 删除会话；4xx / 5xx（含 429）打印 detail 退出。
...
- **预期形态**：上下文窗口是 40 条 `ContextEntry`（用户 + 助手各算一条），第 k 条请求带 2k−1 条历史，第 20 条时 39 条、第 21 条起被截到 40 条，所以曲线应从第 21 条起趋于平稳；平稳值 ≈ 固定 system（粗估 4.9k token）+ 20 轮问答。每日额度 100 条，60 + first-bubble 的 5 次能在同一天跑完。
- **复现步骤**：后端同上一项（真实 Key），`python3 scripts/measure/prompt-tokens.py --api http://localhost:8022 --user demo --password demo123 --character 陈千语 --count 60`。
...
- **退出登录不 abort 进行中的流**：流继续写 store，结束后的重拉因无 token 得到 401（静默）；`StreamingState.controller` 已保留，一行即可接上。
...
- **两项优化数据待补**：首个气泡时间与 prompt_tokens 曲线要接真实 DeepSeek Key 后运行 `scripts/measure/first-bubble.mjs` / `prompt-tokens.py`（第 4 节有命令）；后端 notes 里 4.9k token 只是按字数粗估。
...
- **注释规范的覆盖**：源码里的 11 处 `💡` 注释指向第 3 节的 10 个亮点（`sse`、`sse-persist`、`abort-race`、`flex-layout`、`bubble-measure`、`hover-css`、`preflight-max-width`、`settings-draft`、`sqlite-postgres`、`prompts-backend`）与第 4 节的 `font-subset`；第 3 节另外 7 个亮点（`jwt-401`、`context-window`、`tailwind-theme`、`zoom-canvas`、`contenteditable-emoji`、`test-pyramid`、`git-hooks`）还没有对应的 `💡`（可在 `http.ts` 的 `assertOk`、`chat.py` 第 24 行、`index.css` 第 38 行、`DesignCanvas.tsx`、`ChatInput.tsx`、`playwright.config.ts`、`.husky/pre-commit` 补，注意每文件 ≤ 5 处 emoji）。
...
9. **为什么不用 passlib？** 已停更且与 bcrypt ≥ 4.1 不兼容；直接 `hashpw` / `checkpw` 两行。密码规则限定可打印 ASCII 6–64 位，天然不超过 bcrypt 72 字节，前后端同一条正则。
...
14. **做了哪些有数据的优化？** 字体子集化 4.32 MB → 0.93 MB（−78.5%）；提示词移到后端 + 去依赖让 JS 722.8 → 358.2 KB，产物合计 −69.0%；`memo` 让流式重渲染 348 → 11；hover 0 次渲染。首个气泡时间与 prompt_tokens 曲线脚本已备，等真实 Key。见第 4 节。
